...
1. `npm install` -> Installiert alle benötigten Pakete
2. `example.env` in `./apps/express/api/example.env` Kopieren und in `.env` umbenennen
3. Variablen in `.env` Datei entsprechend anpassen
4. `npx nx run-many -t serve -p express-api web-app` -> Startet Frontend und Backend
5. Frontend:`localhost:4200` Backend: `localhost:4201/api`
